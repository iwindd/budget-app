<div x-data="{
    {{-- INPUT --}}
    from_location_id: @entangle('budgetItemAddressForm.from_id'),
    back_location_id: @entangle('budgetItemAddressForm.back_id'),
    dates: @entangle('budgetItemAddressForm.dates'),
    from_time: @entangle('budgetItemAddressForm.from_time'),
    back_time: @entangle('budgetItemAddressForm.back_time'),
    plate: @entangle('budgetItemAddressForm.plate'),
    distance: @entangle('budgetItemAddressForm.distance'),
    checkbox: @entangle('budgetItemAddressForm.multiple'),
    {{--  --}}
    addressesRaw: @entangle('budgetForm.addresses'),
    ...
    @php
        $addressError = array_merge(
            $errors->get('budgetItemAddressForm.dates', []),
            $errors->get('budgetItemAddressForm.dates.*', []),
        );
    @endphp
    <section class="w-[310]">
        ...
        <main class="flex flex-row lg:flex-col datepicker-address gap-1 pb-2">
            <section class="w-[310]" wire:ignore>
                <textfield id="budgetItemAddressForm.dates" type="hidden" x-ref="datepicker" class="hidden">
            </section>
            <section>
        ...
        <x-checkbox :label="__('address.multiple_dates')" wire:model="budgetItemAddressForm.multiple" />
    </section>
    <section class="flex-grow col-span-4">
        @php
            $root = ['class' => 'lg:col-span-1 md:col-span-2 col-span-4'];
        @endphp
        <form wire:submit="onAddAddress" class="grid grid-cols-4 gap-1">
            <x-selectize :root="$root" :options="$addressSelectize" lang='address.input-from' wire:model="budgetItemAddressForm.from_id" />
            <x-budgets.timepicker :root="$root" lang="address.input-from-datetime" wire:model="budgetItemAddressForm.from_time" />
            <x-selectize :root="$root" :options="$addressSelectize" lang='address.input-back' wire:model="budgetItemAddressForm.back_id" />
            <x-budgets.timepicker :root="$root" lang="address.input-back-datetime" wire:model="budgetItemAddressForm.back_time" />
            <x-textfield :root="$root" lang="address.input-plate" wire:model="budgetItemAddressForm.plate" />
            <x-textfield :root="$root" lang="address.input-distance" wire:model="budgetItemAddressForm.distance" />
            <div>
